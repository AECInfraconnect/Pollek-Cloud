...
- Detail tabs are functional panels, not just visual buttons: Summary, Entities, Relationships, Policies, Telemetry, Alerts, Timeline, Bundle Status, Compliance, Audit, Settings, and Administration.
...
- Administration belongs in the same operational console, with an explicit tenant switcher and visible workflows for SaaS signup, local-dev login/session, role test user seeding, invitations, member role changes, IDP configuration, SCIM provisioning, subscription changes, payment references, invoice previews, billing webhook idempotency tests, and offline license issuance.
- Bundle Status is a separate object-detail view for signed artifacts, manifest readiness, rollout delivery, hot reload, and Cloud-to-Local dispatch evidence.
- Settings is a separate object-detail view for selected object scope, selected tenant context, persistence status, Contract Hub artifacts, and security posture.
