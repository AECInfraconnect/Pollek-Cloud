...
const baseUrl = (process.argv[2] || process.env.POLLEK_CLOUD_URL || "http://127.0.0.1:8790").replace(/\/+$/, "");
...
async function main() {
  ...
  check("contract discovery 200", contract.status === 200,
    contract.payload?.contract_version ? `contract_version=${contract.payload.contract_version}` : `status=${contract.status}`);

  // 2. Baseline fleet (informational; a shared instance may already have data)
  const fleet0 = await api("/api/fleet");
  check("fleet readable", fleet0.status === 200,
  ...
  const ledgerBeforeEnroll = await api(`/v1/tenants/${encodeURIComponent(tenantId)}/lcp/usage-ledgers`, {
    method: "POST",
    body: {
      schema_version: "pollek.lcp.usage-ledger.v1",
      tenant_id: tenantId,
      lcp_id: lcpId,
      usage_entries: [{ agent_id: "a", device_id: deviceId, user_subject: userSubject, provider: "p", model: "m", total_tokens: 1 }]
    }
  });
  check("usage ledger rejected before enroll (gate enforced)", ledgerBeforeEnroll.status === 400,
    `status=${ledgerBeforeEnroll.status}`);

  // 4. Enroll the LCP (register into fleet)
  const enroll = await api("/enroll", {
    method: "POST",
    body: { hostname: deviceId, device_id: deviceId, lcp_id: lcpId, os: "linux", os_family: "linux", os_version: "smoke-test" }
  });
  check("enroll 200", enroll.status === 200, `device_id=${enroll.payload?.device_id ?? "?"}`);
  const fleetAfterEnroll = await api("/api/fleet");
  check("LCP registered in fleet", (fleetAfterEnroll.payload?.local_control_planes || []).some((l) => l.id === lcpId));
  check("tree shows enrolled LCP", (fleetAfterEnroll.payload?.tree || []).some((n) => n.type === "lcp" && n.id === lcpId));

  // 5. Ingest inventory (entity)
  const ingest = await api("/api/entities/ingest", {
    method: "POST",
    body: {
      device_id: deviceId, lcp_id: lcpId, user_subject: userSubject,
      snapshot: { agents: [{ agent_id: `agent_${runId}`, name: `Smoke Agent ${runId}`, trust_level: "trusted" }] }
    }
  });
  ...
  const telemetry = await api("/v1/telemetry/batches", {
    ...
    body: {
      ...
      events: [{
        ...
        payload: {
          agent_id: `agent_${runId}`, agent_name: `Smoke Agent ${runId}`, user_subject: userSubject,
          device_id: deviceId, lcp_id: lcpId, provider: "Anthropic", model: "claude-sonnet-4",
  ...
  check("telemetry batch accepted (stored=1)", telemetry.status === 202 && telemetry.payload?.stored === 1,
  ...
  const telemetryReplay = await api("/v1/telemetry/batches", {
    method: "POST",
    body: {
      schema_version: "telemetry-batch.v1", tenant_id: tenantId, device_id: deviceId, batch_id: `batch_${runId}_replay`,
      events: [{
        schema_version: "telemetry-envelope.v1", event_id: `evt_${runId}_usage`, event_type: "ai_usage_event",
        timestamp: new Date().toISOString(), tenant_id: tenantId, device_id: deviceId, redaction_applied: true,
        payload: { provider: "Anthropic", model: "claude-sonnet-4", tokens: { total_tokens: 1500 } }
      }]
    }
  });
  check("idempotent replay deduped (duplicates=1, stored=0)",
  ...
  const ledger = await api(`/v1/tenants/${encodeURIComponent(tenantId)}/lcp/usage-ledgers`, {
    ...
    body: {
      schema_version: "pollek.lcp.usage-ledger.v1",
      ledger_id: `ledger_${runId}`, tenant_id: tenantId, lcp_id: lcpId, device_id: deviceId,
      os_family: "linux", os_version: "smoke-test", capture_method: "smoke", observed_at: new Date().toISOString(),
      usage_entries: [{
        id: `usage_${runId}`, agent_id: `agent_${runId}`, agent_name: `Smoke Agent ${runId}`,
        device_id: deviceId, user_subject: userSubject, provider: "OpenAI", model: "gpt-5-codex",
        pricing_model: "token_metered", allocation_method: "direct_token_meter",
        call_count: 3, input_tokens: 800, output_tokens: 200, total_tokens: 1000,
        estimated_cost_cents: 90, currency: "USD", confidence: "reported_by_lcp"
      }]
    }
  });
  check("usage ledger accepted after enroll", ledger.status === 202 && ledger.payload?.ledger?.accepted_count === 1,
  ...
  check("ingest-status reflects our batch", Boolean(totals && totals.accepted >= 1),
    `accepted=${totals?.accepted} duplicates=${totals?.duplicates} quarantined=${totals?.quarantined_secrets}`);

  const overview = await api(`/api/reports/cost-tokens/overview?tenant_id=${encodeURIComponent(tenantId)}`);
  const agentGroup = (overview.payload?.categories?.agent || []).find((g) => g.key === `agent_${runId}`);
  check("cost-token overview includes our agent usage", Boolean(agentGroup && agentGroup.total_tokens >= 1500),
    agentGroup ? `agent tokens=${agentGroup.total_tokens} cost_cents=${agentGroup.cost_cents}` : "agent group not found");

  const byUser = await api(`/api/reports/cost-tokens?tenant_id=${encodeURIComponent(tenantId)}&group_by=user`);
  const userGroup = (byUser.payload?.groups || []).find((g) => g.key === userSubject);
  check("cost-token report groups by our user", Boolean(userGroup && userGroup.total_tokens >= 1500),
    userGroup ? `user tokens=${userGroup.total_tokens}` : "user group not found");

  console.log("");
  console.log(`Summary: ${passed} passed, ${failed} failed`);
  console.log(JSON.stringify({ ok: failed === 0, baseUrl, runId, passed, failed, results }, null, 2));
  process.exit(failed === 0 ? 0 : 1);
}

main().catch((error) => {
  console.error(`\nSmoke test could not run against ${baseUrl}: ${error?.message || error}`);
  console.error("If this is a connection error, confirm the URL is reachable from where you run this script.");
  process.exit(1);
});
