...
- `GET/POST /api/entities/watch` manages near-real-time LCP polling and manual refresh.
- The watcher pulls Local Pollek entities and configuration snapshots, fingerprints them, and only broadcasts `local_entities.updated` when data changes.
- Watch fingerprints strip volatile timestamps and latency fields so near-real-time polling does not create false inventory changes.
...
- `/api/fleet` exposes `lcp_watch`, `security_posture`, local configuration snapshots, and cloud-to-local dispatches for UI visibility.
